...
source AnnSet is given, per tile and label the union IoU (UTM) of what was written vs the source
clipped to the tile must reach selfcheck_min_union_iou (catches axis flips and offsets).
"""
...
ROW_BAND_M: Final = 0.05  # polylines are compared as thin bands of this half-width
...
def _shape_utm(shape: CvatShape, tile_id: str) -> BaseGeometry:
    ...
    if shape.tag == "polyline":
        return LineString(pts).buffer(ROW_BAND_M)
...
def _source_union(annset: AnnSet, label: Label, tile_id: str) -> BaseGeometry:
    ...
    clipped = shapely.intersection(shapely.union_all(geoms), tile_box(tile_ref(tile_id)))
    return clipped.buffer(ROW_BAND_M) if label == Label.ROW else clipped
...
def _geometry_issues(annset: AnnSet, doc: CvatDocument, min_iou: float, zip_name: str,
                     skip: frozenset[str]) -> list[Issue]:
    return [i for img in doc.images if img.name[: -len(IMAGE_EXT)] not in skip
            for label in Label for i in _iou_issue(annset, img, label, min_iou, zip_name)]
...
def _check_part(path: Path, doc: CvatDocument, expected_sha256: Mapping[str, str], cfg: CvatExportConfig,
    ...
    if annset is not None and cfg.verify_roundtrip:
        issues += _geometry_issues(annset, read, cfg.selfcheck_min_union_iou, zip_name, skip)
    return names, issues
