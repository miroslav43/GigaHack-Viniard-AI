...
- **Site de prezentare** (ADR-028): vizitatorul fără cont și fără demo vede la `/` pagina `src/app/[locale]/prezentare/page.tsx` (rewrite în `src/proxy.ts`; și la `/prezentare`), construită pe cercetarea din `docs/PREZENTARE_CERCETARE.md` (doar afirmații verificate). Formularul „Solicitați un pilot” scrie în `public.lead` (doar din server, cu cheia secretă); cererile se văd și se urmăresc în `/super-admin?tab=leads`. Capturi: `frontend/public/marketing/`.
- **Asistent** (ADR-027): butonul rotund din dreapta jos (pe fiecare pagină; pe hartă, la stânga butoanelor de zoom) deschide un panou non-modal care explică pașii din aplicație și dă linkuri directe (rămâne deschis la navigare). `POST /api/assistant` → Gemini prin OpenRouter (`OPENROUTER_API_KEY`, `OPENROUTER_MODEL`, implicit `google/gemini-3.8-flash`) sau direct Google (`GEMINI_API_KEY`, `GEMINI_MODEL`), în `frontend/.env.local`, doar server (`src/lib/assistant/llm.ts`); promptul e în `src/lib/assistant/prompt.ts` și citează etichetele din `messages/*.json` (`pnpm test:scripts` le verifică). O funcție nouă în UI se descrie și acolo.
- **Traseu pe fermă** (ADR-028): în panoul unei ferme de pe hartă, „Traseu cel mai scurt”. Alegi startul (= sosirea) cu un click, iar aplicația calculează în browser (Web Worker, `src/lib/farmRoute/`) cel mai scurt tur închis prin toate țintele fermei, pe drumuri și de-a lungul rândurilor. Arată lungimea și durata (4 km/h) și oferă export GPX. Rezultatul e orientativ (UTM 35N plan), nu înlocuiește ruta oficială. Apare doar pe survey-uri cu ferme și drumuri (`siret3`).
