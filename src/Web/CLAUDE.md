...
- **Asistent** (ADR-027): butonul rotund din dreapta jos (pe fiecare pagină; pe hartă, la stânga butoanelor de zoom) deschide un panou non-modal care explică pașii din aplicație și dă linkuri directe (rămâne deschis la navigare). `POST /api/assistant` → Gemini (`GEMINI_API_KEY`, `GEMINI_MODEL` în `frontend/.env.local`, doar server); promptul e în `src/lib/assistant/prompt.ts` și citează etichetele din `messages/*.json` (`pnpm test:scripts` le verifică). O funcție nouă în UI se descrie și acolo.
