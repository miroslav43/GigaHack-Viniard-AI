...
import { SURVEY_ID, dataUrl, getOverlayFiles, getRows, getSummary } from "@/lib/data";
...
export default async function MapPage({ params }: PageProps<"/[locale]/harta">) {
  ...
  const [summary, rows, overlayFiles] = await Promise.all([getSummary(), getRows(), getOverlayFiles()]);
  return (
    <Suspense>
